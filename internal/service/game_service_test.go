...
func TestScheduleDelayedEventPublishesAfterOneMonth(t *testing.T) {
	svc := &GameService{
		engine:        engine.NewGameEngine(),
	...
	if len(snapshot) != 0 {
		t.Fatalf("expected delayed event to stay pending before month check, got %d queued events", len(snapshot))
	}

	svc.checkDelayedEvents()

	snapshot = svc.engine.Bus.Snapshot()
	if len(snapshot) != 1 || snapshot[0].Type != "DELAY_END" {
		t.Fatalf("expected delayed event after one month check, got %+v", snapshot)
	}
}

func TestScheduleDelayedEventPublishesOnlyAfterRequiredMonths(t *testing.T) {
	svc := &GameService{
		engine:        engine.NewGameEngine(),
		silo:          &model.Silo{CurrentYear: 122, CurrentMonth: 1},
		agent:         &model.Agent{},
	...
	svc.checkDelayedEvents()
	if got := len(svc.engine.Bus.Snapshot()); got != 0 {
		t.Fatalf("expected no delayed event after first month check, got %d queued events", got)
	}

	svc.checkDelayedEvents()
	snapshot := svc.engine.Bus.Snapshot()
	if len(snapshot) != 1 || snapshot[0].Type != "DELAY_END" {
		t.Fatalf("expected delayed event after second month check, got %+v", snapshot)
	}
}
